...
public:
	...
	bool isClose(Vec3 position) {
		float thresh = 0.05;
		
		float disx = position[0] - m_position_center[0];
		//if (abs(disx) < thresh + m_dim[0] / 2.0) return true;
		float disy = position[1] - m_position_center[1];
		//if (abs(disy) < thresh + m_dim[1] / 2.0) return true;
		float disz = position[2] - m_position_center[2];
		//if (abs(disz) < thresh + m_dim[2] / 2.0) return true;
		
		return (abs(disz) < thresh + m_dim[2] / 2.0 && abs(disy) < thresh + m_dim[1] / 2.0 && abs(disx) < thresh + m_dim[0] / 2.0);
	}
